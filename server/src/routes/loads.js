...
import { requireAdmin, resolveTruckerFarms } from '../middleware/auth.js';

const router = Router();

// List loads - employees see their farm, truckers see linked farms too
router.get('/', async (req, res) => {
  try {
    let farmIds = [req.farmId];

    // If trucker, also show loads from linked farms
...
// Create load - any authenticated user (employee or admin)
...
router.put('/:id', requireAdmin, async (req, res) => {
  ...
  const { rows } = await pool.query(
    `UPDATE loads SET
     ...
     WHERE id=$13 AND farm_id=$14 AND deleted_at IS NULL RETURNING *`,
    [
