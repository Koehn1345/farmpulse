...
router.post('/', requireAdmin, async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [
      req.farmId, b.type, b.field_id || null,
...
router.put('/:id', requireAdmin, async (req, res) => {
  ...
  const { rows } = await pool.query(
    `UPDATE commodities SET
      type=$1, field_id=$2,
      stack_number=$3, type_of_forage=$4, cutting=$5, bale_count=$6,
      avg_bale_weight_lbs=$7, estimated_stack_tonnage=$8, actual_stack_tonnage=$9,
      type_crop=$10, seed_details=$11, estimated_tons_per_acre=$12,
      estimated_total_tons=$13, actual_tons=$14, actual_tons_per_acre=$15
     WHERE id=$16 AND farm_id=$17 AND deleted_at IS NULL RETURNING *`,
    [
      b.type, b.field_id || null,
