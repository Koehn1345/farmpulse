...
export default function Commodities() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
                ...
                <tr key={row.id} className="table-row cursor-pointer" onClick={() => setViewRow(row)}>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex gap-2">
                      <button className="btn-secondary !px-2 !py-1" onClick={(e) => { e.stopPropagation(); openEdit(row); }}><Pencil size={12} /></button>
                      <button className="btn-danger" onClick={(e) => { e.stopPropagation(); handleDelete(row.id); }}><Trash2 size={12} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {viewRow && (
        <Modal
          title={`Loads — ${viewRow.type === 'Forage' ? (viewRow.stack_number || viewRow.type_of_forage) : viewRow.type_crop}`}
        ...
        >
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800">
                {(viewRow.type === 'Forage' ? ['Date', 'Customer', 'Bales', 'Tons'] : ['Date', 'Customer', 'Tons']).map(h => (
            ...
            <tbody>
              {loads
                .filter(l => l.commodity_id === viewRow.id)
                ...
                .map(l => (
                  <tr key={l.id} className="table-row">
                    ...
                    <td className="px-3 py-2 font-mono text-slate-100">{l.net_weight ? (l.net_weight / 2000).toFixed(2) : '—'}</td>
                  </tr>
                ))}
              {loads.filter(l => l.commodity_id === viewRow.id).length === 0 && (
                <tr><td colSpan={viewRow.type === 'Forage' ? 4 : 3} className="px-3 py-8 text-center text-slate-500">No loads logged against this {viewRow.type === 'Forage' ? 'stack' : 'crop'} yet.</td></tr>
              )}
            </tbody>
          </table>
        </Modal>
